...
if (url.match(/screen=info_player/)) {
	...
	if (coords != null) {
		...
		var index = (localStorage.getItem('RazorCount') != null) ? parseInt(localStorage.getItem('RazorCount')) : 0;
		console.log('index from storage' + localStorage.getItem('RazorCount'));
		if (index >= coords.length) {
			index = 0;
			UI.InfoMessage('This is the last target. Beyond this, cycle will restart', 5000, 'success');
		}
		coords = coords[index];
		index += 1;
		doc.forms[0].input.value = coords;
		...
		insertUnit(doc.forms[0].snob, no);
		insertUnit(doc.forms[0].knight, pal);
		localStorage.setItem('RazorCount', index);
	}
}
